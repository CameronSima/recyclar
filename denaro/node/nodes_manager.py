...
class NodesManager:
    ...
    def init():
        ...
        NodesManager.nodes = NodesManager.db.get(
            'nodes') or ['https://recyclar.still.party']
        NodesManager.last_messages = NodesManager.db.get(
            'last_messages') or {'https://recyclar.still.party': timestamp()}
    ...
    def get_nodes():
        ...
        NodesManager.nodes = [node.strip('/')
                              for node in NodesManager.nodes if len(node)]
    ...
    def get_recent_nodes():
        full_nodes = {node_url: NodesManager.get_last_message(
            node_url) for node_url in NodesManager.get_nodes()}
    ...
    def clear_old_nodes():
        NodesManager.init()
        NodesManager.nodes = [node for node in NodesManager.get_nodes(
