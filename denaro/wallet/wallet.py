...
Database.credentials = {
    'user': os.environ.get('DENARO_DATABASE_USER', 'user'),
    'password': os.environ.get('DENARO_DATABASE_PASSWORD', 'password'),
...
async def main():
    ...
    parser.add_argument('command', metavar='command', type=str,
                        help='action to do with the wallet', choices=['createwallet', 'send', 'balance'])
    ...
    if command == 'createwallet':
        ...
        parser.add_argument('command', metavar='command',
                            type=str, help='action to do with the wallet')
        parser.add_argument('-to', metavar='recipient',
                            type=str, dest='recipient', required=True)
        parser.add_argument('-d', metavar='amount', type=str,
                            dest='amount', required=True)
        parser.add_argument('-m', metavar='message', type=str,
                            dest='message', required=False)
        ...
        try:
            requests.get('http://localhost:3006/push_tx',
                         {'tx_hex': tx.hex()}, timeout=10)
        except Exception as e:
            print(f'Could not push transaction to local node: {e}')
            await denaro_database.add_pending_transaction(tx)
            requests.get('https://recyclar.still.party/push_tx',
                         {'tx_hex': tx.hex()}, timeout=10)
